...
void initSearch() {
    ...
    for (int t = 0; t < 6; ++t)
        for (int depth = 0; depth < 9; ++depth)
            LateMovePruningCounts[t][depth] = roundf((depth > 0 ? t / 2.0 + 2.5 : 0) + depth * depth * (t + 4) / 7.0);
}
...
int search(Thread *thread, PVariation *pv, int alpha, int beta, int depth) {
    ...
    const int moveCountIndex = 2 + MIN(3, MAX(-2, trend));
    ...
    while ((move = selectNextMove(&movePicker, board, skipQuiets)) != NONE_MOVE) {
        ...
        if (   best > -MATE_IN_MAX
            ...
            && movesSeen >= LateMovePruningCounts[moveCountIndex][depth])
            skipQuiets = 1;
