...
uint16_t selectNextMove(MovePicker *mp, Board *board, int skipAllQuiets, int skipRegularQuiets) {
    ...
    switch (mp->stage) {
            ...
            if (mp->noisySize) {
                ...
                if (mp->values[best] >= 0) {
                    ...
                    if (!staticExchangeEvaluation(board, mp->moves[best], mp->threshold)) {
                        mp->values[best] = -1;
                        return selectNextMove(mp, board, skipAllQuiets, skipRegularQuiets);
                    }
        ...
        case STAGE_GENERATE_QUIET:
            ...
            if (!skipAllQuiets && !skipRegularQuiets) {
        ...
        case STAGE_QUIET:
            ...
            if (!skipAllQuiets && !skipRegularQuiets && mp->quietSize) {
