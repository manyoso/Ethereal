...
static int majorHistoryDelta(int depth) {
    // Cap update size to avoid saturation
    return MIN((depth+1)*(depth+1), HistoryMax);
}
...
void updateHistoryHeuristics(Thread *thread, uint16_t *moves, int *scores, int length, int depth, int oldAlpha) {
    ...
    int bonus, majorBonus, colour = thread->board.turn;
    ...
    majorBonus = majorHistoryDelta(depth);

    for (int i = 0; i < length; i++) {

        // Apply a malus until the final move
        int delta = (moves[i] == bestMove) ? bonus : (scores[i] > oldAlpha ? -bonus : -majorBonus);
