...
void updateHistoryHeuristics(Thread *thread, int alpha, uint16_t *moves,
                             int *scores, int length, int depth) {
    ...
    for (int i = 0; i < length; i++) {
        int delta = moves[i] == bestMove ? bonus : scores[i] > alpha ? -bonus/2 : -bonus;
        if (scores[i] > alpha)
            alpha = scores[i];
